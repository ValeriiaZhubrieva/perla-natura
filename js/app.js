(() => {
    ...
    function menuInit() {
        if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
            ...
            if (e.target.closest(".menu__close") || e.target.closest(".menu__link") || e.target.closest(".menu__logo")) menuClose();
        }));
    }
    function menuClose() {
        bodyUnlock();
    ...
    function initSliders() {
        if (document.querySelector(".hero__slider")) new Swiper(".hero__slider", {
            ...
            on: {
                init: function() {
                    const button = document.querySelector(".hero__slider .hero__play");
        ...
        const swiperProposals = document.querySelectorAll(".proposals__slider");
        if (swiperProposals) swiperProposals.forEach((parent => {
            const swiperProposalsParent = parent.parentElement;
            const swiperNextBtn = swiperProposalsParent.querySelector(".swiper-button-next");
            const swiperPrevBtn = swiperProposalsParent.querySelector(".swiper-button-prev");
            swiperProposalsParent.querySelector(".swiper-pagination");
            new Swiper(parent, {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
        ...
        const resizableSwiper = (breakpoint, swiperClass, swiperSettings, callback) => {
            if (swiperClass || document.querySelector(swiperClass)) {
                let swiper;
                ...
                const checker = function() {
                    if (breakpoint.matches) {
                        ...
                        return;
                    } else return enableSwiper(swiperClass, swiperSettings);
                };
                breakpoint.addEventListener("change", checker);
                checker();
            }
        };
        const stepsSlider = document.querySelectorAll(".steps__slider");
        if (stepsSlider) stepsSlider.forEach((parent => {
            const stepsSliderParent = parent.parentElement;
            const swiperNextBtn = stepsSliderParent.querySelector(".swiper-button-next");
            const swiperPrevBtn = stepsSliderParent.querySelector(".swiper-button-prev");
            stepsSliderParent.querySelector(".swiper-pagination");
            resizableSwiper("(max-width: 649.98px)", parent, {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 2,
        ...
        const catalogSlider = document.querySelectorAll(".proposals__slider-catalog");
        if (catalogSlider) catalogSlider.forEach((parent => {
            const catalogSliderParent = parent.parentElement;
            const swiperNextBtn = catalogSliderParent.querySelector(".swiper-button-next");
            const swiperPrevBtn = catalogSliderParent.querySelector(".swiper-button-prev");
            catalogSliderParent.querySelector(".swiper-pagination");
            resizableSwiper("(max-width: 649.98px)", parent, {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
    ...
    function addActiveParentClass(buttonsClass, activeClass, clickNotInside = true) {
        ...
        document.addEventListener("click", (function(e) {
            ...
            if (!isClickInsideButtons && clickNotInside) buttons.forEach((item => {
    ...
    addActiveParentClass(".menu__toggle", "dropdown-open");
    addActiveParentClass(".promocode__btn", "promocode-open", false);
    addActiveParentClass(".summary-products__title", "summary-products-hide", false);
    addActiveParentClass(".summary-products__code-btn", "code-open", false);
    function checkInputFilled(input) {
        const parent = input.parentElement;
        if (input.value.trim() !== "") parent.classList.add("filled"); else parent.classList.remove("filled");
    }
    ...
    document.addEventListener("click", (function(e) {
        ...
        if (targetElement.closest('[class*="__viewpass"]')) {
            const inputElement = targetElement.parentElement.querySelector("input");
            if (inputElement) {
